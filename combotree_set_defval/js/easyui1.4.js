...
var _maxheight=_57c.height();
if(wrap.parent().hasClass('full_screen_max')){
    _maxheight=_57a-31;  //全屏：设置视图高度
    view.height(_maxheight);
}else{
...
if(!$.data(_588,"datagrid").panel.parent().hasClass('full_screen_max')){//全屏
    _58c.height(_590);
    _58d.height(_590);
...
//20161206-丁以发，默认排序改为倒序
var _5d4=col.order||"desc";
...
if(col){
    var _728 = "";
    if (_727.indexOf('.') > -1) {
        _728 = _725[_727.split('.')[0]][_727.split('.')[1]];
    } else {
        _728 = _725[_727];
    }
...
//增加title:true
var _title="";
if(col.showtip==true&&typeof(_728)!="undefined"&&_728!==null&&_728!=''){
    _title="title=\""+_728+"\"";
}
cc.push("<td field=\""+_727+"\" "+cls+" "+_72b+" "+_title+ ">");
var _72b="";
if(!col.checkbox){
if(col.align){
        ...
        if (option instanceof Array) {
            $.each(option, function (i, item) {
                if (item.field) {
                    return jq.each(function () {
                        var $panel = $(this).datagrid("getPanel");
                        var $field = $('td[field=' + item.field + ']', $panel);
                        if ($field.length) {
